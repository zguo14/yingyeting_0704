...
function clickRecordSearch(iType) {
    var szDeviceIdentify = $("#ip").val(),
        ...
        // 减8小时搜索
        szStartTime=szStartTime.replace(" ", ",").replace(/-/g, ",").replace(/:/g, ",");
        szEndTime=szEndTime.replace(" ", ",").replace(/-/g, ",").replace(/:/g, ",");
        var sta=szStartTime.split(",");
        var endt=szEndTime.split(",");
        var st=new Date(sta[0],sta[1].replace(/\b(0+)/gi,""),sta[2].replace(/\b(0+)/gi,""),sta[3].replace(/\b(0+)/gi,""),sta[4].replace(/\b(0+)/gi,""),sta[5].replace(/\b(0+)/gi,""));
        st.setHours(st.getHours()-8);
        var str=st.getFullYear()+"-"+st.getMonth()+"-"+st.getDate()+" "+st.getHours()+":"+st.getMinutes()+":"+st.getMilliseconds();
        var end2=new Date(endt[0],endt[1].replace(/\b(0+)/gi,""),endt[2].replace(/\b(0+)/gi,""),endt[3].replace(/\b(0+)/gi,""),endt[4].replace(/\b(0+)/gi,""),endt[5].replace(/\b(0+)/gi,""));
        end2.setHours(end2.getHours()-8);
        var end2str=end2.getFullYear()+"-"+end2.getMonth()+"-"+end2.getDate()+" "+end2.getHours()+":"+end2.getMinutes()+":"+end2.getMilliseconds();
        ...
        success: function (xmlDoc) {
            ...
            if ("MORE" === $(xmlDoc).find("responseStatusStrg").eq(0).text()) {
                ...
                for(var i = 0, nLen = $(xmlDoc).find("searchMatchItem").length; i < nLen; i++) {
                    ...
                    szStartTime=szStartTime.replace("T", ",").replace("Z", "").replace(/-/g, ",").replace(/:/g, ",");
                    szEndTime=szEndTime.replace("T", ",").replace("Z", "").replace(/-/g, ",").replace(/:/g, ",");
                    var sta=szStartTime.split(",");
                    var endt=szEndTime.split(",");
                    var st=new Date(sta[0],sta[1].replace(/\b(0+)/gi,""),sta[2].replace(/\b(0+)/gi,""),sta[3].replace(/\b(0+)/gi,""),sta[4].replace(/\b(0+)/gi,""),sta[5].replace(/\b(0+)/gi,""));
                    st.setHours(st.getHours()+8);
                    ...
                    var objTd = objTr.insertCell(0);
                    objTd.id = "downloadTd" + (i + g_iSearchTimes * 40);
                    ...
                    //console.log('BB'+i + g_iSearchTimes * 40);
                }
                ...
                for(var i = 0; i < iLength; i++) {
                    ...
                    $("#downloadTd" + (i + g_iSearchTimes * 40)).data("fileName", szFileName);
                    $("#downloadTd" + (i + g_iSearchTimes * 40)).data("playbackURI", szPlaybackURI);
                    //console.log('CC'+i + g_iSearchTimes * 40);
                }
...
function clickStartDownloadRecord(i) {
    ...
    // var szStartTime = $("#starttime").val().replace(/-/g,"").replace(/:/g,"");
    // var  szEndTime = $("#endtime").val().replace(/-/g,"").replace(/:/g,"");
    // var startt=szStartTime.split(" ");
    // var endt=szEndTime.split(" ");
    //console.log('下载录像：'+szPlaybackURI);
    // rtsp://192.168.1.64/Streaming/tracks/101/?starttime=20200409T001521Z&endtime=20200409T002359Z
    //var split=szPlaybackURI.split("?");
    //szPlaybackURI=split[0]+'?starttime='+startt[0]+'T'+startt[1]+'Z&endtime='+endt[0]+'T'+endt[1]+'Z';
    //console.log('下载录像：'+szPlaybackURI);
